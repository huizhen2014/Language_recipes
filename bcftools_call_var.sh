...
bcftools mpileup -C 50 -P ILLUMINA -a "FORMAT/AD" -a "FORMAT/ADF" -a "FORMAT/ADR" -a "FORMAT/DP" -a "FORMAT/SP" -R $bed -f $ref -O b ${sample}.sorted.dedup.bam > ${sample}.bcftools.bcf.gz
...
cat > "${sample}.ped"<<EOF
SMA_Family_Li	${sample}	0	0	1	2
EOF
#
bcftools call --ploidy GRCh37 -m -v -S "${sample}.ped" -R $bed -O z ${sample}.bcftools.bcf.gz -o ${sample}.bcftools.combined.raw.vcf.gz
#
#rm -rf ${sample}.ped
